function HeatIndex(){
    const TA = document.getElementById("TA");//nome=TA, propriedade=valor, valor=document.getElementbyId("TA"), tempo=pre-processamento 
    var aux_tmp = TA.value;
    aux_tmp = aux_tmp.replace(/,/g, '.');
    aux_tmp = parseFloat(aux_tmp); //nome=parseFloat, propriedade=implementação, tempo=design
    if((aux_tmp%1)!=0)
        aux_tmp = aux_tmp.toFixed(1);
    document.getElementById("TA").value = aux_tmp;
    const RH = document.getElementById("RH");//nome=document, propriedade=endereço, tempo=carregamento
    var aux_rh = RH.value;//nome=aux_rh, propriedade=valor, valor=RH.value, tempo=execução
    ...
    if(aux_tmp>100){
        aux_tmp=100;//nome=aux_tmp, propriedade=valor, valor=100, tempo=execução
    ...
    if(isNaN(aux_tmp)||isNaN(aux_rh))//nome=isNaN, propriedade=semântica, tempo=compilação
...
function Tocar(){
    var ic = HeatIndex(); //nome=ic, propriedade=endereço, tempo=execução
    const TA = document.getElementById("TA");
    var tmp = parseFloat(TA.value);
    const RH = document.getElementById("RH");
    var ur = parseFloat(RH.value);
    var audiotmp = new Audio("audio/temperatura.ogg"); //nome=Audio(), propriedade=semântica, tempo=compilação
    var audiorh = new Audio("audio/umidade.ogg"); //nome=audiorh, propriedade=tipo, valor=Audio(), tempo=execução 
